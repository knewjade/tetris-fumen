...
export class Field {
    ...
    constructor({ field = Field.create(FieldConstants.PlayBlocks), garbage = Field.create(FieldConstants.Width) }: {
        field?: PlayField,
        garbage?: PlayField,
    }) {
        this.field = field;
    ...
    add(x: number, y: number, value: number): void {
        if (0 <= y) {
            this.field.add(x, y, value);
        } else {
            this.garbage.add(x, -(y + 1), value);
        }
    }
    ...
    copy(): Field {
        return new Field({ field: this.field.copy(), garbage: this.garbage.copy() });
    }
